...
public class MediaButtonIntentReceiver extends DaggerBroadcastReceiver {
                ...
                if (action == KeyEvent.ACTION_DOWN) {
                    if (down) {
                        if ((MediaButtonCommand.TOGGLE_PAUSE.equals(command) ||
                                MediaButtonCommand.PLAY.equals(command)) &&  (lastClickTime != 0 && eventTime - lastClickTime > LONG_PRESS_DELAY)) {
    ...
    static void startService(Context context, String command) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && MediaButtonCommand.PAUSE.equals(command) && (MusicServiceConnectionUtils.serviceBinder == null || MusicServiceConnectionUtils.serviceBinder.getService() == null)) {
                return;
