...
class QueueFragment : BaseFragment(), QueueContract.View {
    ...
    @Inject lateinit var application: ShuttleApplication

    @Inject lateinit var requestManager: RequestManager

    @Inject lateinit var multiSheetSlideEventRelay: MultiSheetSlideEventRelay

    @Inject lateinit var playerPresenter: PlayerPresenter

    @Inject lateinit var queuePresenter: QueuePresenter
    ...
    private lateinit var itemTouchHelperCallback:
            com.simplecity.amp_library.ui.views.recyclerview.ItemTouchHelperCallback
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        disposables.add(
                Aesthetic.get(context).colorPrimary().subscribe { color ->
        ...
        // In landscape, we need to adjust the status bar's translation depending on the slide
        // offset of the sheet
        if (ShuttleUtils.isLandscape(application)) {
            ...
            disposables.add(
                    multiSheetSlideEventRelay
                            .events
                            .filter { multiSheetEvent ->
                                multiSheetEvent.sheet == MultiSheetView.Sheet.SECOND
                            }
                            .filter { multiSheetEvent -> multiSheetEvent.slideOffset >= 0 }
                            .subscribe { multiSheetEvent ->
                                statusBarView.translationY =
                                        (1 - multiSheetEvent.slideOffset) *
                                                ResourceUtils.toPixels(16f)
                            }
            )
        }
    ...
    private val songClickListener =
            object : QueueViewBinder.ClickListener {
                ...
                override fun onQueueItemLongClick(
                        position: Int,
                        queueViewBinder: QueueViewBinder
                ): Boolean {
                    return cabHelper.handleLongClick(queueViewBinder, queueViewBinder.queueItem)
                }

                override fun onQueueItemOverflowClick(
                        position: Int,
                        view: View,
                        queueViewBinder: QueueViewBinder
                ) {
                ...
                override fun trackInfoChanged(song: Song?) {
                    if (song != null) {
                        ...
                        if (song.albumArtistName != null && song.albumName != null) {
                            line2.text =
                                    String.format("%s • %s", song.albumArtistName, song.albumName)
                        }
                    }
                }
                ...
                val queueHeaderView =
                        QueueHeaderView(
                                StringUtils.makeSongsAndTimeLabel(
                                        ...
                                        queueItems.size,
                                        queueItems
                                                .map { queueItem -> queueItem.song.duration / 1000 }
                                                .sum()
                                )
                        )
    ...
    override fun updateQueuePosition(queuePosition: Int) {
        ...
        MultiSheetView.getParentMultiSheetView(view)?.let { multiSheetView ->
            ...
            // If we're not currently displaying the queue, then scroll to keep the position up to
            // date
        ...
        queueViewBinders.firstOrNull { queueViewBinder -> queueViewBinder.isCurrentTrack }?.let {
                previouslySelectedQueueViewBinder ->
    ...
    override fun onRemovedFromQueue(queueItem: QueueItem) {
        adapter.items
                .first { viewBinder ->
                    viewBinder is QueueViewBinder && viewBinder.queueItem == queueItem
                }
                ?.let { adapter.removeItem(it) }
    }

    override fun onRemovedFromQueue(queueItems: List<QueueItem>) {
        adapter.items
                .filter { viewBinder ->
                    viewBinder is QueueViewBinder && queueItems.contains(viewBinder.queueItem)
                }
                .forEach { adapter.removeItem(it) }
    }
    ...
    override fun showCreatePlaylistDialog(songs: List<Song>) {
        CreatePlaylistDialog.newInstance(mediaManager.queue.toSongs())
    ...
    inner class ItemTouchHelperCallback :
            com.simplecity.amp_library.ui.views.recyclerview.ItemTouchHelperCallback(
                    ...
                    { pos ->
                        queuePresenter.removeQueueItem(
                                (adapter.items[pos] as QueueViewBinder).queueItem
                        )
                    }
            ) {
        override fun onMove(
