...
public class TaggerTask extends AsyncTask<Object, Integer, Boolean> {
	...
	public TaggerTask(Context context, boolean showAlbum, boolean showTrack, List<String> paths,
			List<DocumentFile> documentFiles, String titleText, String albumText, String artistText,
			String albumArtistText, String yearText, String trackText, String trackTotalText, String discText,
			String discTotalText, String lyricsText, String commentText, String genreText,
	...
	protected Boolean doInBackground(Object... params)  {
		...
		for (int i = 0; i < paths.size(); i++) {
			...
			try (File orig = new File(path); AudioFile audioFile = AudioFileIO.read(orig);) {
				...
				if (tagUpdate.hasChanged()) {
					...
					if (requiresPermission && temp != null) {
						...
						if (documentFile != null) {
							...
							if (cleanUp(temp.toPath()) && (tempFiles.contains(temp))) {
				...
				publishProgress(i);
				success = true;
			} catch (CannotWriteException | IOException | CannotReadException | InvalidAudioFrameException
					| TagException | ReadOnlyFileException | NoSuchFileException | DirectoryNotEmptyException e) {
				e.printStackTrace();
			} finally {
	...
	public TaggerTask build() {
		return new TaggerTask(applicationContext, showAlbum, showTrack, paths, documentFiles, titleText, albumText,
				artistText, albumArtistText, yearText, trackText, trackTotalText, discText, discTotalText, lyricsText,
				commentText, genreText, tagCompletionListener);
	}
}