    ...
    public void onResume() {
        ...
        if (!settingsManager.getUsePalette() && !settingsManager.getUsePaletteNowPlayingOnly()) {
            disposables.add(getAestheticColorSetDisposable().subscribe(
                    localColorSet -> animateColors(PlayerFragment.this.colorSet, localColorSet, 800, this::invalidateColors, null),
                    error -> {
                        // Nothing to do
                    })
            );
    ...
    private SimpleTarget<ColorSet> paletteTarget = new SimpleTarget<ColorSet>() {
        ...
        public void onLoadFailed(Exception e, Drawable errorDrawable) {
            ...
            getAestheticColorSetDisposable()
                    .take(1)
                    .subscribe(
                        localColorSet -> animateColors(PlayerFragment.this.colorSet, localColorSet, 800, intermediateColorSet -> invalidateColors(intermediateColorSet), null),
                            error -> {
                                // Nothing ot do
                            }
                    );
