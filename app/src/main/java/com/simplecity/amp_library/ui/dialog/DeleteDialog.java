...
public class DeleteDialog extends DialogFragment implements SafManager.SafDialog.SafResultListener {
        ...
        switch (type) {
            case Type.ARTISTS:
                return Observable.fromIterable(artists)
                        ...
                        .reduce(Collections.<Song>emptyList(), (localSongs, localSongs2) -> Stream.concat(Stream.of(localSongs), Stream.of(localSongs2)).toList())
    ...
    void deleteSongsOrShowSafDialog() {
        disposables.add(getSongs().map(localSongs -> {
            // Keep track of the songs we want to delete, for later.
            Stream.of(localSongs).forEach(song -> {
