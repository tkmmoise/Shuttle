...
class AlbumDetailFragment :
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (ShuttleUtils.canDrawBehindStatusBar()) {
            toolbar.layoutParams.height =
                    (ActionBarUtils.getActionBarHeight(context!!) +
                                    ActionBarUtils.getStatusBarHeight(context!!))
                            .toInt()
            toolbar.setPadding(
                    toolbar!!.paddingLeft,
                    (toolbar.paddingTop + ActionBarUtils.getStatusBarHeight(context!!)).toInt(),
        ...
        toolbar_layout.setExpandedTitleTypeface(
                TypefaceManager.getInstance().getTypeface(context, TypefaceManager.SANS_SERIF_LIGHT)
        )
        toolbar_layout.setCollapsedTitleTypeface(
                TypefaceManager.getInstance().getTypeface(context, TypefaceManager.SANS_SERIF)
        )
        ...
        disposables.add(
                Aesthetic.get(context).colorPrimary().compose(distinctToMainThread()).subscribe {
                        primaryColor ->
    ...
    private fun setupToolbarMenu(toolbar: Toolbar) {
        ...
        SongSortHelper.updateSongSortMenuItems(
                toolbar.menu,
                sortManager.albumDetailSongsSortOrder,
                sortManager.albumDetailSongsAscending
        )
    }
    ...
    private fun loadBackgroundImage() {
        ...
        requestManager
                .load<ArtworkProvider>(album as ArtworkProvider?)
                // Need to override the height/width, as the shared element transition tricks Glide
                // into thinking this ImageView has
                // the same dimensions as the ImageView that the transition starts with.
                // So we'll set it to screen width (plus a little extra, which might fix an issue on
                // some devices..)
                .override(width, height)
                .diskCacheStrategy(DiskCacheStrategy.SOURCE)
                .priority(Priority.HIGH)
                .placeholder(
                        PlaceholderProvider.getInstance(context)
                                .getPlaceHolderDrawable(album.name, true, settingsManager)
                )
                .centerCrop()
        ...
        if (contextualToolbar != null) {
            ...
            contextualToolbar.setOnMenuItemClickListener(
                    SongMenuUtils.getSongMenuClickListener(
                            Single.defer {
                                Operators.reduceSongSingles(contextualToolbarHelper!!.items)
                            },
                            presenter
                    )
            )

            contextualToolbarHelper =
                    object :
                            ContextualToolbarHelper<Single<List<Song>>>(
                                    context!!,
                                    contextualToolbar,
                                    object : ContextualToolbarHelper.Callback {

                                        override fun notifyItemChanged(
                                                viewModel: SelectableViewModel
                                        ) {
                                            val index =
                                                    adapter.items.indexOf(viewModel as ViewModel<*>)
                                            if (index >= 0) {
                        ...
                        override fun start() {
                            super.start()
                            // Need to hide the collapsed text, as it overlaps the contextual
                            // toolbar
                        ...
                        override fun finish() {
                            if (toolbar_layout != null &&
                                            collapsingToolbarTextColor != null &&
                                            collapsingToolbarSubTextColor != null
                            ) {
                                toolbar_layout.collapsedTitleTextColor =
                                        collapsingToolbarTextColor!!
                                toolbar_layout.collapsedSubTextColor =
                                        collapsingToolbarSubTextColor!!
                            }
    ...
    private val enterSharedElementCallback =
    ...
    private var songClickListener: SongView.ClickListener =
            object : SongView.ClickListener {
                override fun onSongClick(position: Int, songView: SongView) {
                    if (!contextualToolbarHelper!!.handleClick(
                                    songView,
                                    Single.just(listOf(songView.song))
                            )
                    ) {
                ...
                override fun onSongLongClick(position: Int, songView: SongView): Boolean {
                    return contextualToolbarHelper!!.handleLongClick(
                            songView,
                            Single.just(listOf(songView.song))
                    )
                }
                ...
                override fun onSongOverflowClick(position: Int, v: View, song: Song) {
                    ...
                    popupMenu.setOnMenuItemClickListener(
                            SongMenuUtils.getSongMenuClickListener(song, presenter)
                    )
                    popupMenu.show()
                }
    ...
    override fun setData(data: MutableList<Song>) {
        ...
        if (data.isNotEmpty()) {
            ...
            viewModels.add(
                    SubheaderView(
                            StringUtils.makeSongsAndTimeLabel(
                                    context!!,
                                    data.size,
                                    data.map { song -> song.duration / 1000 }.sum()
                            )
                    )
            )
                    ...
                    ArrayList<ViewModel<*>>(
                            data
                                    .map { song ->
                                        ...
                                        songView.setShowTrackNumber(
                                                songsSortOrder ==
                                                        SortManager.SongSort.TRACK_NUMBER ||
                                                        songsSortOrder ==
                                                                SortManager.SongSort.DETAIL_DEFAULT
                                        )
            ...
            if (album.numDiscs > 1 &&
                            (songsSortOrder == SortManager.SongSort.DETAIL_DEFAULT ||
                                    songsSortOrder == SortManager.SongSort.TRACK_NUMBER)
            ) {
                var discNumber = 0
        ...
        setItemsDisposable =
                adapter.setItems(
                        viewModels,
                        object : CompletionListUpdateCallbackAdapter() {
