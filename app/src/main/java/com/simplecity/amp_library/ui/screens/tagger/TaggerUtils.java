...
public class TaggerUtils {
    ...
    /**
     * Checks the passed in paths to see whether the file at the given path is
     * available in our
     * document tree. If it is, and we have write permission, the document file is
     * added to the
     ...
     */
    static boolean hasDocumentTreePermission(Context context, SettingsManager settingsManager,
            List<DocumentFile> documentFiles, List<String> paths) {

        boolean hasDocumentTreePermission = false;

        String treeUri = getDocumentTree(context, settingsManager);
        if (treeUri == null) {
            // We don't have any document tree at all - so we're not going to have
            // permission for any files.
            return false;
        }

        // Find the file in the document tree. If it's not there, or it doesn't have
        // permission,
    ...
    static void showChooseDocumentDialog(Context context, MaterialDialog.SingleButtonCallback listener,
            boolean hasChecked) {
        MaterialDialog.Builder builder = new MaterialDialog.Builder(context)
                .title(R.string.edit_tags)
                .content(hasChecked ? R.string.tag_editor_document_tree_permission_failed
                        : R.string.tag_editor_document_tree_message)
